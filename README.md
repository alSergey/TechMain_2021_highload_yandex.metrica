...
* Админку будем писать на TS. TypeScript будем использовать из-за его дополнительных возможностей по сравнению с JS + из-за статической типизации, которая позволяет предотвратить много ошибок.

* Для удобства будем использовать фреймворк Rect/Vue/Angular. Фреймворк будем выбирать в зависимости от того, что лучше знает наша команда.
...
* Для бекенда будем использовать микросервисную архитектуру. Она посзволит нам распределить нагрузку и легко масштабироваться в будущем.
* Бекенд можно будет разделить на сервис авторизации, сервис отдачи метрик, сервис регистрации сайтов, который так же будет взаимодействовать с другими сервисами.
* Бекенд админки будем писать на GO, потому что он из коробки поддерживает многопоточность, достаточно быстрый и простой.

#### Backend метрик

Бекенд митрик должен получать данные и передавать distributed table, который дальше уже будет распределять данные между шардами.
...
* Для логирования фронта можно использовать Graylog или Sentry.
* Для логгирования бекенда будем использовать Graylog.

#### Мониторинг

* Для мониторинга производительности фронтенда и сбора ошибок будем использовать Sentry.
* Для мониторинга бекенда будем использовать prometheus + grafana.

### Kubernetes

Для фронтенда и бекенда будем использовать kubernetes. В нем легко можно поднять несколько сервисов и настроить балансировку между ними для распределения нагрузки.

## 6. Схема проекта

### Админка

![Админка](./assets/schema_adminka.png)
