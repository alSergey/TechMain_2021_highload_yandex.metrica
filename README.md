...
3. **Средний размер хранилища пользователя** - за пользователя будем считать владельца сайта
    ...
    * Метрики сайта - будем считать, что метрики сайта - это пользователи и информация о их посещениях. Распишем какая информация у нас есть у пользователя при посещении сайта. Разобьем информацию о пользователя на общие данные пользователя и данные за каждое посещение.
    ...
    | Информация о клике         | 20                        | 100 Б  |
    | Посещенная страница        | 10                        | 50 Б   |
    | Общий размер               |                           | 220 Б  |

    Для удобства будем считать размер пользователя за визит 250 Б. Будем считать, что пользователь заходит на сайт в среднем 10 раз в день, получается размер метрики пользователя за день 4000 Б. Возьмем количество пользователей на сайте за день 10 000.
...
4. **Среднее количество действий пользователя**
    ...
    * Сбор метрик
        ...
        * Нажатие на кнопку - мы посчитали, что во время посещения сайта пользователь нажимает на кнопки примерно 20 раз, получается 200 запросов в день
        * Переход на другую страницу - мы посчитали, что пользователь во время посещения сайта переходит между 10 страницами, получается 100 запросов в день.
    ...
    | Заход на сайте              | 10                  |
    | Нажатие на кнопку           | 200                 |
    | Переход на другую страницу  | 100                 |
    | Уход с сайта                | 10                  |
    | Общее количество            | 331                 |
...
1. **Размер хранения в разбивке по типам данных**
    ...
    * Метрики сайта - у нас 120 млн сайтов, получаем за 1 год 1000 ПБ. Яндекс Метрика существует с 2009 года, то есть 12 лет, учитывая, что сайтов раньше было меньше, то будем считать 3000 ПБ за все время
...
3. RPS и трафик в секунду
    ...
    | Нажатие на кнопку           | 19 МБ/C  | 694444  |
    | Переход на другую страницу  | 9 МБ/C   | 347222  |
    | Уход с сайта                | 1 МБ/C   | 34722   |
    | **Итог**                    | 163 МБ/C | 1124998 |
